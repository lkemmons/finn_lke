...
def _instrument_from_filename(path: Path) -> str | None:
    """Try to infer the instrument from a FIRMS-style filename.

    Recognises both NRT and archive naming conventions:

      MODIS  →  MODIS_C6_1_*.txt, MCD14DL*, fire_archive_M-C61_*.csv
      VIIRS  →  SUOMI_VIIRS_*, VNP14IMGTDL_NRT_*, fire_archive_SV-C2_*.csv,
                fire_archive_J1V-C2_*.csv (NOAA-20 / J1 VIIRS)

    Returns None if no pattern matches; the caller falls back to whatever's
    in the data.
    """
    name = path.name.lower()
    if any(t in name for t in ("modis", "mcd14", "m-c6")):
        return "MODIS"
    if any(t in name for t in ("viirs", "vnp14", "suomi", "sv-c", "j1v", "jpss")):
        return "VIIRS"
    return None
...
def _read_one(path: Path) -> gpd.GeoDataFrame:
    ...
    # If we can identify the instrument from the filename, trust it and
    # apply to every row.  FIRMS files (both NRT and archive) are
    # almost always per-instrument with predictable names — far more
    # reliable than the in-data `instrument` column, which can be
    # missing or malformed on some rows.  Files with unrecognised names
    # fall back to per-row detection in step1.prep.
    inst = _instrument_from_filename(path)
    if inst is not None:
        gdf["instrument"] = inst
        log.info("  instrument inferred from filename: %s", inst)
